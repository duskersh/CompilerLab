...
void findLeaders()
{
	...
	for(map<ll,string>::iterator it=instList.begin();it!=instList.end();it++)
	{
		if(flag)
		{
			...
			if(ind!=-1)
			{
				...
				ll ind1=it->second.rfind("goto");
				//if(ind1!=-1)
				//	leaders.insert(to_int(it->second.substr(ind1+5)));
				//cout<<"\n"<<"second substring finding:"<<it->second.substr(ind+5);
				flag=true;
				continue;
			}
			flag=false;
	...
	cout<<"leaders are:";
	for (set<ll> :: iterator itr = leaders.begin(); itr != leaders.end(); ++itr)
    {
        cout << '\t' << *itr;
    }
    cout << endl;
...
void genBasicBlock()
{
	...
	for(map<ll,string>::iterator it=instList.begin();it!=instList.end();it++)
	{
		...
		if((ind=it->second.rfind("goto"))!=-1)
		{
	       ...
	       //bb
			//bbinst1=it->second.substr(0,ind+5) + to_string(lineToBlock[to_int(it->second.substr(ind+5))]);
			
		}
		basicBlock[bbno].push_back(bbinst);
	    basicBlock[bbno].push_back(bbinst1);	
	}
}
